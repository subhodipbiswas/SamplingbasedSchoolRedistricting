...
from .bounds import Bounds, UpperBound
import numpy
...
def better_balanced_than_initial(
        initial_partition, threshold=0, pop_key="population", target_key="capacity"
):
    """
    Added by Subhodip Biswas
    """
    if threshold >= 1:
        threshold *= 0.01

    def _target_imbalance(partition):
        imbalance = [abs(1 - partition[pop_key][d] / partition[target_key][d]) if partition[target_key][d] > 0 else 1
                     for d in partition[pop_key].keys()]
        return sum(imbalance) / len(imbalance)

    initial_imbalance = _target_imbalance(initial_partition)

    return UpperBound(_target_imbalance, bound=initial_imbalance + threshold)
...
def districts_within_tolerance(partition, attribute_name="population", percentage=0.1):
    ...
    :param attribute_name: string that is the name of an updater in partition
...
# The following code segments have been added by Subhodip Biswas <subhodip@vt.edu>
# for making GerryChain work on the school redistricting problem
def no_centerless_districts(partition, target_attribute="capacity"):
    return all(partition[target_attribute][d] > 0
               for d in partition[target_attribute].keys())
